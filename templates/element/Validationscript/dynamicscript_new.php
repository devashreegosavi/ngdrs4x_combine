        ...
        foreach ($listcontrol as $controltype => $valrule) {
            ...
            foreach ($rulearr as $singlerule) {
                ...
                foreach ($result_codes as $errorkey => $error_record) {
                    if ($error_record->error_code == $singlerule) {
                        if ($error_record->is_autocomplete_flag == 'Y') {
                            $autoflag = 1;
                        }
                    }
                }
    ...
    foreach ($fieldlist as $listkey => $listcontrol) {//$listcontrol->key(select,text) and correspondent value rule for key is(is_alpha,..)
        foreach ($listcontrol as $controltype => $valrule) {
            ...
            foreach ($rulearr as $singlerule) {
                ...
                foreach ($result_codes as $errorkey => $error_record) {
                    if ($error_record->error_code == $singlerule) {
                        $regex = $error_record->pattern_rule_client; // GET MATCHING RULE PATTERN
...
function generate_logic_checkbox($listkey, $singlerule, $regex, $result_codes, $lang, $formid = NULL) {
    foreach ($result_codes as $errorkey => $error_record) {
        if ($error_record->error_code == $singlerule) {
            ?> var regex =<?php echo $error_record->pattern_rule_client; ?>; // GET MATCHING RULE PATTERN
            <?php
        }
    }
    ?>
...
function generate_logic_textbox_event($listkey, $rulearr, $result_codes, $lang, $formid = NULL) {
    ...
    foreach ($rulearr as $singlerule) {
        foreach ($result_codes as $errorkey => $error_record) {
            if ($error_record->error_code == $singlerule) {
                ?>  var regex =<?php echo $error_record->pattern_rule_client; ?>; // GET MATCHING RULE PATTERN                    
                <?php if ($error_record->maxlength_flag == 'Y' && is_numeric($error_record->maxlength_value)) { ?>
                            if ($('#<?php echo $listkey; ?>').val().length > <?php echo $error_record->maxlength_value ?>){
                            $('#<?php echo $listkey; ?>').val($('#<?php echo $listkey; ?>').val().substr(0,<?php echo $error_record->maxlength_value ?>));
                            }
                <?php } ?>
...
function message_display_logic($listkey, $singlerule, $error_codes, $lang) {
    foreach ($error_codes as $errorkey => $error_record) {
        if ($error_record->error_code == $singlerule) {
            ?>
                    $('#<?php echo $listkey; ?>_error').html('<?php echo $error_record->error_messages_en; ?>');
                    $('.<?php echo $listkey; ?>_error').css("display", "block");
                    $("#<?php echo $listkey; ?>").css("border-color", "#ced4da");
...
function message_remove_logic($listkey) {
    ?>
            ...
            $('.<?php echo $listkey; ?>_error').css("display", "none");
            $("#<?php echo $listkey; ?>").css("border-color", "#ced4da");
    <?php
}
