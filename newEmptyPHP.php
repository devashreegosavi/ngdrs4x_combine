...
//        $this->Auth->allow('welcomenote', 'login', 'add', 'Disclaimer', 'index', 'index1', 'index2', 'registration', 'checkuser', 'viewsingle', 'ViewRegisteruser', 'get_district_name', 'get_captcha', 'aboutus', 'contactus', 'insertuser', 'checkorg', 'sponsordetail_pdf', 'checkcaptcha', 'checkemail', 'send_sms', 'empregistration');
        ...
        $this->Auth->allow('viewnoc_partydetails_in_ngdrs', 'getnocview', 'viewnoc_in_ngdrs', 'getnocvalue', 'check_female_exemption', 'forgotpassword_citizen', 'welcomenote', 'citizenlogin', 'login', 'add', 'Disclaimer', 'index', 'index1', 'index2', 'registration', 'checkuser', 'viewsingle', 'ViewRegisteruser', 'get_district_name', 'get_captcha', 'aboutus', 'contactus', 'insertuser', 'checkorg', 'sponsordetail_pdf', 'checkcaptcha', 'checkemail', 'send_sms', 'empregistration', 'district_change_event', 'otpsavecitizen');
...
//                                                echo 11;exit;
    ...
    public function citizenlogin_old() {
        try {
...
//                                                echo 11;exit;
                                        ...
                                        if ($this->Auth->user('authetication_type') == '1') {
                                            ...
                                            $this->set('count', 0);
                                            $count = ClassRegistry::init('getUserRolecitizen')->query("select count(*) from (select distinct B.module_name,B.url,A.role_id from ngdrstab_mst_userroles_citizen A
                                                        inner join ngdrstab_mst_module B on A.module_id=B.module_id where A.user_id=? order by B.module_name) as role", array($userid));
                                            //check multiple 
                                            //pr($count);exit;
                                            if ($count[0][0]['count'] > 1) {
                                                $usermodules = ClassRegistry::init('getUserRolecitizen')->query("select distinct B.module_name,B.url,A.role_id from ngdrstab_mst_userroles_citizen A
                                                        inner join ngdrstab_mst_module B on A.module_id=B.module_id where A.user_id=? order by B.module_name", array($userid));
                                                $this->set('usermodules', $usermodules);
                                                $this->Session->write("session_redirect", 'welcomemodel');
                                                $this->redirect(array('action' => 'welcomemodel'));
                                            } else {
                                        ...
                                        $this->loadModel('CitizenUser');
                                        $this->CitizenUser->insertUnsuccessfulLogin($user_id);
                                        Cache::clear();
                                        clearCache();
                                        $this->Session->setFlash(__('Authentication Failed'));
                                        $this->redirect(array('controller' => 'Citizenentry', 'action' => 'citizenlogin'));
                ...
                if (!empty($checkuser)) {
                    if ($checkuser[0][0]['mobile_no']) {
                        ...
                        if (!empty($event)) {
                            if ($event[0]['smsevent']['send_flag'] == 'Y') {
//login citizen
                                $this->smssend2(1, $checkuser[0][0]['mobile_no'], $otp, $checkuser[0][0]['user_id'], 4);
                            } else {
                                echo 1;
                                exit;
                            }
                                    ...
                                    if ($dbrecord[0]['CitizenUser']['mobile_no']) {
                                        ...
                                        if (!empty($event)) {
                                            if ($event[0]['smsevent']['send_flag'] == 'Y') {
                                                ...
                                                $this->smssend1(2, $dbrecord[0]['CitizenUser']['mobile_no'], $otp, $dbrecord[0]['CitizenUser']['user_id'], 7);
...
//                               pr($lastotp);exit;
    ...
    public function citizenlogout() {
        try {
            ...
            if($this->Session->read("sess_rtps_trans_id")!='' && $this->Session->read("sess_response_url")!=''){
                //echo 'in';
                $redirect_to_url = $this->Session->read("sess_response_url");
                //pr($redirect_to_url);//exit;
                $this->loadModel('User');
                $this->loadModel('CitizenUser');
                $useridd = $this->Auth->user('user_id');
                $user12 = $this->User->find('first', array('conditions' => array('User.user_id' => array($useridd))));
    ...
    public function genernal_info_old() {
        try {
            ...
            //////////////// Supervisor modifications - by devashree
            $supervisory_flag = $this->conf_reg_bool_info->field('conf_bool_value', array('reginfo_id' => 170));
            $this->set('supervisory_flag', $supervisory_flag);
            /////////////////////////////////////////////////////
        } catch (Exception $ex) {
            $this->Session->setFlash(
                    __('Record Cannot be displayed. Error :' . $ex->getMessage())
    ...
    public function genernal_info() {
        try {
            ...
            array_map(array($this, 'loadModel'), array('genernal_info', 'conf_reg_bool_info', 'ApplicationSubmitted'));
            $this->Session->write("user_role_id", $this->Auth->user('role_id'));
            $this->Session->write("citizen_user_id", $this->Auth->user('user_id'));
            $user_id = $this->Session->read("citizen_user_id");
            $session_tokenval = $this->Session->read("Selectedtoken");
            ...
            //pr($this->Session->read("sess_rtps_trans_id"));
            //pr($this->Session->read("sess_response_url"));
            $statusrecord = $this->genernal_info->get_alldocument($user_id, $laug, $this->Session->read("session_usertype"));
            // pr($statusrecord);
            $arrayone = array();
            for ($i = 0; $i < sizeof($statusrecord); $i++) {
                $tkn = $statusrecord[$i][0]['token_no'];
                // pr($tkn);
                $arrayone[$i]['token_no'] = $tkn;
                $appsub = $this->ApplicationSubmitted->query("select * from ngdrstab_trn_application_submitted where token_no='$tkn'");
                //pr($appsub);
                if (sizeof($appsub) > 0) {
                    for ($j = 0; $j < sizeof($appsub); $j++) {

                        $arrayone[$i]['check_in_flag'] = $appsub[$j][0]['check_in_flag'];
                    }
                } else {
                    $arrayone[$i]['check_in_flag'] = 'NS';
                }
            }
            //pr($arrayone);
            $this->set('arrayone', $arrayone);
            if ($this->Session->read("session_usertype") == 'C') {
                $this->Session->write("manual_flag", 'N');
            }
            $this->set('statusrecord', $statusrecord);
    ...
    public function genernalinfoentry($csrftoken = Null, $flag = '') {
        ...
        try {
            ...
            array_map(array($this, 'loadModel'), array('User', 'DocumentRegTypes', 'foldertobedeleted', 'finyear', 'counter', 'ApplicationSubmitted', 'proceduretype', 'party_entry', 'regconfig', 'uploaded_file_trn', 'file_config', 'District', 'taluka', 'conf_article_feerule_items', 'language', 'mainlanguage', 'articaledepfields', 'genernalinfoentry', 'doc_levels', 'doc_title', 'State', 'User', 'articletrnfields', 'article', 'documenttitle', 'document_execution_type', 'article_screen_mapping', 'article_fee_rule'));
            ...
            $mortgage_days = $this->regconfig->find('first', array('conditions' => array('reginfo_id' => 206)));
            if (isset($mortgage_days['regconfig']['info_value'])) {
                $mortgage_days = '-' . $mortgage_days['regconfig']['info_value'] . 'd';
            } else {
                $mortgage_days = '+0d';
            }

            $this->set('mortgage_day', $mortgage_days);
            ...
            $hide_field_flag = $this->regconfig->find('first', array('conditions' => array('reginfo_id' => 157)));
            if (!empty($hide_field_flag)) {
                $this->set('hide_field_flag', $hide_field_flag['regconfig']['conf_bool_value']);
            }
...
//          pr($submission);exit;
            ...
            $this->set(compact('language', 'language2', 'document_execution_type', 'article', 'documenttitle', 'TemplateMain'));
            ...
            // for cidco
            if ($this->Session->read("role_type") == 'C') {

                if ($this->Session->read("cidco_citizen_flag") == 'N') {
                    $scheme = ClassRegistry::init('scheme_master')->find('list', array('fields' => array('scheme_id', 'scheme_name_' . $doc_lang), 'conditions' => array('role_id' => $this->Session->read("user_role_id"))));
                } else if ($this->Session->read("cidco_citizen_flag") == 'Y') {
                    $record = $this->get_cidco_contract_details($this->Session->read("contract_number"));

                    if (!empty($record)) {
                        $scheme = ClassRegistry::init('scheme_master')->find('list', array('fields' => array('scheme_id', 'scheme_name_' . $doc_lang), 'conditions' => array('scheme_sap_id' => $record->MT_CIDCO_NIC_INTEGRATION_IB->LS_OUTPUT1->item->SCH_ID)));
                    } else {
                        $scheme = ClassRegistry::init('scheme_master')->find('list', array('fields' => array('scheme_id', 'scheme_name_' . $doc_lang), 'order' => array('scheme_sap_id' => 'ASC')));
                    }
                } else {
                    $scheme = array();
                }
            ...
            $this->set('ereg', $ereg = $this->regconfig->field('conf_bool_value', array('reginfo_id' => 170)));
...
//pr($errarr);
            ...
            $docregtypetype = $this->DocumentRegTypes->find('list', array('fields' => array('DocumentRegTypes.registration_type_id', 'DocumentRegTypes.registration_type_desc_' . $laug), 'conditions' => array('display_flag' => 'Y'), 'order' => array('DocumentRegTypes.registration_type_desc_' . $laug => 'ASC')));
            ...
            $taluka = ClassRegistry::init('taluka')->find('list', array('fields' => array('taluka.taluka_id', 'taluka.taluka_name_' . $laug)));
            // print_r($taluka);exit;
            $villagelist = ClassRegistry::init('VillageMapping')->find('list', array('fields' => array('VillageMapping.village_id', 'VillageMapping.village_name_' . $laug)));
            ...
            $this->set('office', $office);
            $this->set('docregtypetype', $docregtypetype);
            ...
            if ($this->request->is('post') || $this->request->is('put')) {
                ...
                // this is for normal flow
                $this->request->data['genernalinfoentry']['org_type_id'] = 4;
                ...
                if (is_numeric($this->Session->read('Selectedtoken'))) {
                    ...
                    if ($lastoffice != $office_id) {
                        ...
                        $this->ApplicationSubmitted->deleteAll(['token_no' => $this->Session->read("Selectedtoken")]);
...
//pr($this->request->data['genernalinfoentry']);

                if ($this->request->data['genernalinfoentry']['registration_type_id'] != 1) {
                    unset($fieldlist['no_of_pages']);
                    unset($fieldlist['exec_date']);

                    $this->request->data['genernalinfoentry']['doc_execution_type_id'] = null;
                }
                ...
                if (!isset($this->request->data['genernalinfoentry']['stamp_flag']) || $this->request->data['genernalinfoentry']['stamp_flag'] == 'N') {
                    unset($fieldlist['stamp_paper_number']);
                }

                if (isset($this->request->data['genernalinfoentry']['stamp_flag']) && $this->request->data['genernalinfoentry']['stamp_flag'] == 'Y') {
                    $checkdupstamp = array();
                    if (is_numeric($tokenval)) {
                        $checkdupstamp = $this->genernalinfoentry->query("select * from ngdrstab_trn_generalinformation where stamp_paper_number=? and token_no NOT IN (?)", array($this->request->data['genernalinfoentry']['stamp_paper_number'], $tokenval));
                    } else {
                        $checkdupstamp = $this->genernalinfoentry->query("select * from ngdrstab_trn_generalinformation where stamp_paper_number=? ", array($this->request->data['genernalinfoentry']['stamp_paper_number']));
                    }
                    if (!empty($checkdupstamp)) {
                        $this->Session->setFlash(
                                __('Entered stamp number already with us ( Stamp Number : ' . $checkdupstamp[0][0]['stamp_paper_number'] . ',Token Number : ' . $checkdupstamp[0][0]['token_no'] . ' )')
                        );
                ...
                // pr($this->request->data['genernalinfoentry']);//exit;
...
//exit;
                ...
                if ($flag == 1) {
                    ...
                    if (!isset($this->request->data['genernalinfoentry']['general_info_id'])) {
                        $result_token = $this->genernalinfoentry->query("select * from generate_token_number()");
                        $this->request->data['genernalinfoentry']['token_no'] = $result_token[0][0]['generate_token_number']; 
...
//                                array('fin_year_id' => $finyear, 'token_no_count' => $rem)); //fields to update
                    ...
                    if ($this->Session->read("session_usertype") == 'O') {
                        ...
                        $this->request->data['genernalinfoentry']['registration_type_id'] = 1;
                        $this->Session->write('e_reg', 'N');
                   
                    if (!is_numeric($this->Session->read('Selectedtoken'))) {

                        $this->request->data['genernalinfoentry']['token_created_userid'] = $this->Auth->user('user_id');
                    }
                    //pr($this->request->data['genernalinfoentry']);
//                    exit;
                    if ($this->genernalinfoentry->save($this->request->data['genernalinfoentry'])) {
                        ...
                        if ($gen_info) {
                            ...
                            $this->Session->write('major_minor', $gen_info[0]['genernalinfoentry']['major_minor_flag']); //madhuri
                            $this->Session->write('reg_type_id', $gen_info[0]['genernalinfoentry']['registration_type_id']); //madhuri
...
//                        pr($this->request->data);exit;
                        ...
                        $prop_applicable = $this->articletrnfields->savedependent_field($laug, $this->request->data['genernalinfoentry'], $this->Session->read('Selectedtoken'), $fields, $this->Session->read("session_usertype"));

                        if ($prop_applicable == 'Y') {
                            $this->Session->write('prop_applicable', 'Y');
                            $this->genernalinfoentry->updateAll(array('property_applicable' => "'Y'"), //fields to update
                                    array('token_no' => $this->Session->read("Selectedtoken"))  //condition
                            );
                        } else {
                            $this->Session->write('prop_applicable', 'N');
                            $this->genernalinfoentry->updateAll(array('property_applicable' => "'N'"), //fields to update
                                    array('token_no' => $this->Session->read("Selectedtoken"))  //condition
                            );
                            $ids = ClassRegistry::init('property_details_entry')->find('all', array('fields' => array('property_id'), 'conditions' => array('token_no' => $this->Session->read('Selectedtoken'))));
//                               pr($ids);
//                               exit;
                            if (count($ids) > 0) {
                                foreach ($ids as $id) {
...
//                        exit;
                ...
                if ($gen_info != NULL) { // set data  to form
                    ...
                    if ($this->request->data['genernalinfoentry']['stamp_paper_date']) {
                        $this->request->data['genernalinfoentry']['stamp_paper_date'] = date('d-m-Y', strtotime($gen_info[0]['genernalinfoentry']['stamp_paper_date']));
                    }
...
//                    exit;
                    ...
                    $this->Session->write('reg_type_id', $gen_info[0]['genernalinfoentry']['registration_type_id']); //madhur
                    $this->Session->write('major_minor', $gen_info[0]['genernalinfoentry']['major_minor_flag']); //madhuri
                    if ($gen_info[0]['genernalinfoentry']['registration_type_id'] == 2 || $gen_info[0]['genernalinfoentry']['registration_type_id'] == 4) {
                        $this->Session->write('e_reg', 'Y');
                    } else {
                        $this->Session->write('e_reg', 'N');
                    }
    ...
    function set_value_tosave_generalinfo($data, $user_id, $stateid, $user_type) {
        try {
            ...
            $data['stamp_paper_date'] = (@$data['stamp_paper_date']) ? date('Y-m-d', strtotime(str_replace('/', '-', $data['stamp_paper_date']))) : NULL;
...
//----------------------------------END EDIT-----------------------------------------------------------------------------------------------------    
            ...
            $property_list_new = $this->property_details_entry->query("SELECT
prop.property_id,
prop.val_id,
dist.district_id,
dist.district_name_$lang,
taluka.taluka_name_$lang,
village.village_name_$lang,
village.village_id,
article.article_desc_$lang,
level1.level_1_desc_$lang,   
level1list.list_1_desc_$lang,
array_to_string(
					ARRAY(
						select evalrule_desc_$lang from ngdrstab_mst_evalrule_new where evalrule_id IN
								(select rule_id from ngdrstab_trn_valuation_details where val_id IN
										(select val_id from ngdrstab_trn_property_details_entry as prop1 where prop1.village_id=prop.village_id and prop1.property_id=prop.property_id)
...
array_to_string(
					ARRAY(
						select concat(eri_attribute_name_en,' : ',paramter_value) from ngdrstab_mst_attribute_parameter as attr
						JOIN   ngdrstab_trn_parameter as trnp ON trnp.paramter_id=attr.attribute_id and trnp.property_id=prop.property_id
					)
...
array_to_string(
					ARRAY(
						select paramter_value from ngdrstab_mst_attribute_parameter as attr
						JOIN   ngdrstab_trn_parameter as trnp ON trnp.paramter_id=attr.attribute_id and trnp.property_id=prop.property_id where attr.attribute_id='205'
					)
...
array_to_string(
					ARRAY(
						select concat(usage_param_desc_$lang,' : ',item_value,' ',unit_desc_$lang) from ngdrstab_trn_valuation_details 
inner join ngdrstab_mst_usage_items_list on ngdrstab_mst_usage_items_list.usage_param_id=ngdrstab_trn_valuation_details.item_id
and area_field_flag='Y'
left join ngdrstab_mst_unit on ngdrstab_mst_unit.unit_id=ngdrstab_trn_valuation_details.area_unit
where val_id=prop.val_id and  item_value>0
					)	
			,' , ') as area,
                        
(select round(sum(final_value)) from ngdrstab_trn_valuation_details where val_id=prop.val_id and item_type_id=2) as val
                         
 FROM  ngdrstab_trn_property_details_entry prop 
 ...
 LEFT JOIN ngdrstab_mst_location_levels_1_property as level1 ON level1.level_1_id=prop.level1_id
 LEFT JOIN ngdrstab_mst_loc_level_1_prop_list as level1list ON level1list.prop_level1_list_id=prop.level1_list_id
 JOIN ngdrstab_trn_generalinformation gen on gen.token_no=prop.token_no
...
where 
  ...
  prop.token_no=?
", array($token));
            $this->set('property_list_new', $property_list_new);

            $report_id = array();
            $report_id = [1, 2];
            $this->loadModel('ReportLabel');
            $rptlabels = $this->ReportLabel->find('list', array('fields' => array('label_id', 'label_desc_' . $this->Session->read("sess_langauge")), 'conditions' => array('report_id in' => $report_id)));
            $this->set('rptlabels', $rptlabels);
            ...
            if ($this->request->is('post') || $this->request->is('put')) {
                ...
                if($this->request->data['propertyscreennew']['property_type_id']!=2){
                    unset($fieldlist['noc_number']);
                    unset($fieldlist['noc_date']);
                }
...
//pr($errors);exit;
                if ($this->ValidationError($errors)) {
                    ...
                    if (isset($this->request->data['propertyscreennew']['noc_date']) && !empty($this->request->data['propertyscreennew']['noc_date'])) {
                        $this->request->data['propertyscreennew']['noc_date'] = date('Y-m-d', strtotime($this->request->data['propertyscreennew']['noc_date']));
                    }
...
// Property Details modify field list
    public function modifyfieldlist($fieldlist, $data) {
        try {
            ...
            if (!isset($json2array['level1list']) || empty($json2array['level1list']) || $json2array['level1list'] == NULL || !isset($fieldlist['level1_list_id'])) {
                if (isset($this->request->data['propertyscreennew']['level1_list_id']) && !is_numeric($this->request->data['propertyscreennew']['level1_list_id'])) {
    ...
    public function edit_propertydetails($prop_id = NULL) {
        try {
            ...
            if (is_numeric($prop_id)) {
                ...
                if (isset($this->request->data['propertyscreennew']['noc_date']) && !empty($this->request->data['propertyscreennew']['noc_date'])) {
                    $this->request->data['propertyscreennew']['noc_date'] = date('d-m-Y', strtotime($this->request->data['propertyscreennew']['noc_date']));
                }
...
//                pr($json2array);exit;
    ...
    public function add_property_attribute() {
        try {
            $this->loadModel("regconfig");
            $regconf_attr = $this->regconfig->find("all", array('conditions' => array('reginfo_id' => 172, 'is_boolean' => 'Y', 'conf_bool_value' => 'Y')));
            $this->check_csrf_token_withoutset($_POST['csrftoken']);
            $file = new File(WWW_ROOT . 'files/jsonfile_' . $this->Auth->user('user_id') . '.json');
            $json = $file->read(true, 'r');
            $json2array = json_decode($json, TRUE);
            ...
            if ($sro_appr_flag == 'Y') {
                if (is_numeric($this->Session->read('Selectedtoken'))) {
                    ...
                    if ($sro_approval == 'Y') {
                        ...
                        if ($this->Session->read("manual_flag") == 'Y') {
                            $result = $this->minorfunction->find("all", array('conditions' => array('manual_reg_flag' => 'Y', 'delete_flag' => 'N', 'marriage_reg_flag' => 'N'), 'order' => array('mf_serial ASC')));
                        } else {
                            $result = $this->minorfunction->find("all", array('conditions' => array('citizen_flag' => 'Y', 'delete_flag' => 'N', 'marriage_reg_flag' => 'N'), 'order' => array('mf_serial ASC')));
                        }
                    } else {
                        $result = $this->minorfunction->find('all', array('conditions' => array('citizen_flag' => 'Y', 'delete_flag' => 'N', 'id' => array(1, 9), 'marriage_reg_flag' => 'N'), 'order' => 'mf_serial ASC'));
                    }
                } else {
                    $result = $this->minorfunction->find('all', array('conditions' => array('citizen_flag' => 'Y', 'delete_flag' => 'N', 'id' => array(1, 9), 'marriage_reg_flag' => 'N'), 'order' => 'mf_serial ASC'));
                }
...
//                } 
                $home_visit_flag = $this->conf_reg_bool_info->field('conf_bool_value', array('reginfo_id' => 177));
                if ($home_visit_flag == 'Y') {
                    if (is_numeric($this->Session->read('Selectedtoken'))) {
                        $party_home_visit = ClassRegistry::init('party_entry')->field('home_visit_flag', array('token_no' => $this->Session->read("Selectedtoken"), 'home_visit_flag' => 'Y'));
                        if (!empty($party_home_visit)) {
                            if ($this->Session->read("manual_flag") == 'Y') {
                                $result = $this->minorfunction->find("all", array('conditions' => array('manual_reg_flag' => 'Y', 'delete_flag' => 'N', 'mf_serial NOT IN(12)', 'marriage_reg_flag' => 'N'), 'order' => array('mf_serial ASC')));
                            } else {
                                $result = $this->minorfunction->find("all", array('conditions' => array('citizen_flag' => 'Y', 'delete_flag' => 'N', 'mf_serial NOT IN(12)', 'marriage_reg_flag' => 'N'), 'order' => array('mf_serial ASC')));
                            }
                        } else {
                            if ($this->Session->read("manual_flag") == 'Y') {
                                $result = $this->minorfunction->find("all", array('conditions' => array('manual_reg_flag' => 'Y', 'delete_flag' => 'N', 'marriage_reg_flag' => 'N'), 'order' => array('mf_serial ASC')));
    ...
    public function data_entry_status() {
        try {
            array_map(array($this, 'loadModel'), array('article_doc_map','genernalinfoentry', 'property_details_entry', 'party_entry', 'conf_reg_bool_info', 'witness', 'appointment', 'ApplicationSubmitted', 'leaseandlicense', 'stamp_duty', 'CitizenPaymentEntry', 'uploaded_file_trn', 'identification', 'article_screen_mapping', 'BankPayment', 'valuation_details'));
...
//pr($partytype_name);exit;
            ...
            //number of witness
            $witness_count = $this->conf_reg_bool_info->find('first', array('conditions' => array('reginfo_id' => 404)));

            if ($witness_count['conf_reg_bool_info']['conf_bool_value'] == 'Y') {
                $witness = $this->witness->find("all", array('conditions' => array('token_no' => $token)));
                if (count($witness) < $witness_count['conf_reg_bool_info']['info_value']) {
                    $status['7'] = 0;
                } else {
                    $status['7'] = 1;
                }
            ...
            $upload_status= $this->article_doc_map->find("all", array('conditions' => array('article_id' => $article_id)));
            $counttwo = count($upload_status);
...
//------------------------------------------------------------------------------------------------------------------------------------------------
                if ($sd_update_result == 1) {
                    ...
                    $this->redirect(array('action' => 'payment', $this->Session->read('csrftoken')));
...
//          pr($submission);exit;
    ...
    public function appointment($csrftoken = NULL) {
        try {
            ...
            // pr($this->Session->read("sess_response_url"));
            //load Model
            array_map(array($this, 'loadModel'), array('genernalinfoentry', 'officeshift', 'party_entry', 'smsevent', 'office', 'appointment', 'ApplicationSubmitted', 'regconfig', 'OfficeShiftMapping'));
            $regconfig = $this->regconfig->find('first', array('conditions' => array('reginfo_id' => 44)));
            $tatkal = $this->regconfig->find('first', array('conditions' => array('reginfo_id' => 53)));
            $startapp = $this->regconfig->find('first', array('conditions' => array('reginfo_id' => 76)));
            $reschedule = $this->regconfig->find('first', array('conditions' => array('reginfo_id' => 78)));

            //////////check for super user - by devashree
            /* $superuser=$this->regconfig->find('first', array('conditions' => array('reginfo_id' => 170)));
              $superuserflg=$superuser['regconfig']['conf_bool_value'];
              if($superuserflg=='Y'){
              $superf='N';
              $suflag=$this->genernalinfoentry->find('all',array('conditions'=>array('genernalinfoentry.token_no'=>$this->Session->read('Selectedtoken'))));
              if(count($suflag)>0){
              $superf=$suflag[0]['genernalinfoentry']['supervisor_verify_flag'];
              }
              if($superf=='N'){
              $this->Session->setFlash("Application is pending for Supervisor verification; after verification you can take appointment");
              return $this->redirect('genernal_info');
              }
              } */
            ////////////////////////////////////////////
...
//            $officeshift = $this->officeshift->find('list', array('fields' => array('shift_id', 'desc_' . $doc_lang), 'order' => array('shift_id' => 'ASC'), 'conditions' => array('shift_id' => $office[0]['office']['shift_id'])));
            ...
            $officeshift = $this->officeshift->find('list', array('fields' => array('shift_id', 'desc_' . $doc_lang), 'order' => array('shift_id' => 'ASC'),
                'conditions' => array('shift_id' => $this->OfficeShiftMapping->find('list', array('fields' => array('shift_id', 'shift_id'),
                        'conditions' => array('office_id' => $office[0]['office']['office_id']))))));
...
", array($this->Session->read('Selectedtoken')));
            ...
            // saddam code start
            $this->set('stoday', Date('Y-m-d'));
            $this->set('slastday', Date('Y-m-d', strtotime("+" . $regconfig['regconfig']['info_value'] . " days")));
// saddam code end
            ...
            if ($this->request->is('post')) {
                ...
                if ($reschedule['regconfig']['conf_bool_value'] == 'Y') {
                    if (isset($this->request->data['appointment']['reschedule_flag']) && $this->request->data['appointment']['reschedule_flag'] == 'Y') {
                        $this->appointment->deleteAll(['token_no' => $this->Session->read("Selectedtoken")]);
                    }
                }
                ...
                $errarr = $this->validatedata($this->request->data['appointment'], $fieldlist);
                $token_no_exist = $this->appointment->find('first', array('conditions' => array('token_no' => $this->Session->read("Selectedtoken"))));
                if (empty($token_no_exist)) {
                    if ((isset($_POST['slot']) && in_array($_POST['slot'], $_SESSION['iterval_array']))) {
                        if ($this->ValidationError($errarr)) {
                            //pr($errarr); exit;
                            $this->request->data['appointment']['user_type'] = $this->Session->read("session_usertype");
                            if (!isset($_POST['slot']) || $_POST['slot'] == '') {
                            ...
                            $check = $this->appointment->find('first', array('conditions' => array('interval_id' => $interval, 'slot_no' => $slot, 'office_id' => $office_id, 'appointment_date' => date('Y-m-d', strtotime($this->request->data['appointment']['appointment_date'])), 'flag' => 'N')));
                            ...
                            if (empty($check)) {
                                ...
                                if ($this->appointment->save($data)) {
                                    ...
                                    //$message = 'Office name:-' . $officename . '  Date:- ' . ' ' . $data['appointment_date'] . '  Time:-' . ' ' . $data['sheduled_time'] . 'Your Pre-registration number is ' . $this->Session->read("Selectedtoken");
                                    $message1 = 'Your document submitted successfully to ' . $officename . ' with PreReg No. ' . $this->Session->read("Selectedtoken") . '. Appointment details ' . $data['appointment_date'] . ' ' . $data['sheduled_time'] . '.-NGDRS Portal';
//pr($message1);pr($event);
                                    if (!empty($event)) {
                                        if ($event[0]['smsevent']['send_flag'] == 'Y') {
                                            ...
                                            if (!empty($seller)) {
                                                $mobno = $seller[0]['party_entry']['mobile_no'];
                                                $this->smssend_msg(3, $mobno, $message1, $this->Session->read("citizen_user_id"), 2);
//                                                $this->smssend(3, $mobno, $message, $this->Session->read("citizen_user_id"), 2);
                                            }
                                            ...
                                            if (!empty($buyer)) {
                                                $mobno1 = $buyer[0]['party_entry']['mobile_no'];
                                                $this->smssend_msg(3, $mobno1, $message1, $this->Session->read("citizen_user_id"), 2);
                                            }
                                            // pr($buyer);exit;
                                        }
                                    }
                    ...
                    $this->Session->setFlash(__("Slot Already Allocated "));
...
//                    exit;
    ...
    public function slot_alocation() {
        try {
            $this->check_csrf_token_withoutset($_POST['csrftoken']);
            if (isset($_POST['office_id']) and is_numeric($_POST['office_id']) and isset($_POST['shift_id']) and is_numeric($_POST['shift_id'])) {
                array_map(array($this, 'loadModel'), array('officeshift', 'office', 'appointment', 'regconfig', 'User', 'genernalinfoentry'));
                $office_id = ClassRegistry::init('ApplicationSubmitted')->field('office_id', array('token_no' => $this->Session->read("Selectedtoken")));
                $stateid = $this->Auth->User("state_id");
                $tatkal = $this->regconfig->find('first', array('conditions' => array('reginfo_id' => 53)));

                // changed
                $info = $this->genernalinfoentry->find('first', array('conditions' => array('token_no' => $this->Session->read("Selectedtoken"))));
                $_POST['office_id'] = NULL;
                if (!empty($info)) {
                    $_POST['office_id'] = $info['genernalinfoentry'] ['office_id'];
                }
                ...
                $sro_user_result = $this->User->query("select count(user_id) from ngdrstab_mst_user where role_id=? and office_id=?", array(999901, $office_id));
                ...
                $shift = $this->officeshift->find('all', array('order' => array('officeshift.shift_id' => 'ASC'),
                    'joins' => array(
                        array('table' => 'ngdrstab_mst_office_shift_mapping', 'alias' => 'shift_mapping', 'conditions' => array("shift_mapping.office_id=" . $_POST['office_id']))
                    ),
                    'conditions' => array('shift_mapping.shift_id' => $_POST['shift_id'])));
                ...
                if ($virtual_office_flag == 'Y') {
                    ...
                    $appointment = $this->appointment->find('all', array('conditions' => array(
                            'appointment.appointment_date ' => date('Y-m-d', strtotime($_POST['app_date'])), 'appointment.office_id' => trim($office_id), 'appointment.shift_id' => trim($_POST['shift_id']), 'appointment.state_id' => $stateid, 'appointment.flag' => 'N'
                        ), 'joins' => array(
...
//            pr('hello');exit;
            ...
            if (!empty($regconf)) {
                if (!empty($json2array['prop_attributes_seller'])) {
                    echo 'Y';
                } else {
                    echo 'N';
                }
    ...
    function payment($csrftoken = NULL, $payment_id = NULL) {
        ...
        try {
            ...
            array_map(array($this, 'loadModel'), array('stamp_duty', 'ReceiptCounter', 'exemption_article_rules', 'article', 'fees_calculation', 'fees_calculation_detail', 'external_links', 'payment_mode', 'bank_master', 'PaymentPreference', 'CitizenPaymentEntry', 'article_fee_items', 'PaymentFields', 'payment', 'genernalinfoentry', 'OnlinePayment'));
            $this->set('actiontypeval', NULL);
            ...
            if (is_numeric($payment_id)) {
                ...
                if (!empty($info) && $info['genernalinfoentry']['last_status_id'] == 1) {
                    if ($this->CitizenPaymentEntry->deleteAll(array('payment_id' => $payment_id, 'token_no' => $token))) {
                        $this->OnlinePayment->query("delete from ngdrstab_trn_online_payment_received where citizen_payment_id=? and token_no=? ", array($payment_id, $token));
                        $this->OnlinePayment->query("delete from ngdrstab_trn_payment_details  where record_lock='N' and citizen_payment_id=? and token_no=? ", array($payment_id, $token));
                        $this->Session->setFlash(__("Record Deleted Successfully"));
                    } else {
                        $this->Session->setFlash(__("Document is submitted its can't be  Deleted "));
                    }
                }
            ...
            $citizen_payment = $this->CitizenPaymentEntry->get_all_payment($token, $lang);
            ...
            $this->set(compact('payment_mode', 'exemption_flag', 'exemption_amount', 'payment_url', 'lang', 'stamp_duty_details', 'payment', 'accounthead', 'paymentfields', 'feedetails', 'citizen_payment', 'paymentfields_trn'));
            ...
            if ($this->request->is('post')) {
                ...
                if ($this->ValidationError($errors)) {
                    ...
                    $checkpayment_mode = $this->payment_mode->find("first", array('conditions' => array('payment_mode_id' => $this->request->data['payment']['payment_mode_id'])));
                    //pr($checkpayment_mode);
                    //exit;
                    if (!empty($checkpayment_mode)) {
                        if ($checkpayment_mode['payment_mode']['verification_flag'] == 'Y') {
                            $usertype = $this->Session->read("session_usertype");
                            ...
                            } else if ($request_data['payment_mode_id'] == 7) {  // 7. esbtr
                                $ServiceResponse = $webserviceobj->ESBTRVerification($request_data, $extrafields);
                            ...
                            } else if ($request_data['payment_mode_id'] == 15) {//sbi epay
                                $ServiceResponse = $webserviceobj->SbiepayVerification($request_data, $extrafields);
                            } else if ($request_data['payment_mode_id'] == 17) {//sbi epay
                                $ServiceResponse = $webserviceobj->RazorpayVerification($request_data, $extrafields);
                            } else if ($request_data['payment_mode_id'] == 18) {//JK  Bank
                                $ServiceResponse = $webserviceobj->RazorpayVerification($request_data, $extrafields);
                            } else {
                                $ServiceResponse['Error'] = 'Payment Mode Not FOund!';
                            }

                            // pr($ServiceResponse);exit;
                            if (empty($ServiceResponse['Error'])) {
                                if (!empty($ServiceResponse['OnlinePaymentData'])) {
                                    $this->CitizenPaymentEntry->save($ServiceResponse['OnlinePaymentData']);
                                    $this->Session->setFlash(__("Payment  Verified  Successfully!"));
                                    $citizen_payment_id = $this->CitizenPaymentEntry->getLastInsertID();
                                    if (!empty($ServiceResponse['PaymentData'])) {
                                        foreach ($ServiceResponse['PaymentData'] as $key => $paymentdetails) {

                                            if (is_numeric($key)) {
                                                $ServiceResponse['PaymentData'][$key]['citizen_payment_id'] = $citizen_payment_id;
                                            } else {
                                                $ServiceResponse['PaymentData']['citizen_payment_id'] = $citizen_payment_id;
                                            }
                                        }
                                        //pr($ServiceResponse['PaymentData']);exit;
                                        $this->payment->SaveAll($ServiceResponse['PaymentData']);
                                        $ServiceResponse['OnlinePaymentData']['citizen_payment_id'] = $citizen_payment_id;
                                        if (!empty($ServiceResponse['OnlinePaymentData'])) {
                                    ...
                                    $this->Session->setFlash(__("Payment Not  Verified "));
                                }
                            } else {
                            ...
                            ////////////Added for BY AMIT to display payment in SRO login payment tab directly////////
//                            $this->request->data['payment']['user_type']= $this->Session->read("session_usertype");
//                            $ofcidn=$this->CitizenPaymentEntry->query("Select office_id from ngdrstab_trn_generalinformation where token_no=".$this->request->data['payment']['token_no']);
//                            $this->request->data['payment']['office_id']=$ofcidn[0][0]['office_id'];                           
                            //////////////////////////////////////////////////////////////////////////////////////////
                            if ($this->CitizenPaymentEntry->save($this->request->data['payment'])) {
                                ////////////Added for BY AMIT to display payment in SRO login payment tab directly////////
//                                $this->payment->Save($this->request->data['payment']);
//                                ///////////////AMIT TO hide account Head///////
                                $citizen_payment_id = $this->CitizenPaymentEntry->getLastInsertID();
                                $request_data = $this->request->data['payment'];
                                $request_data['citizen_payment_id'] = $citizen_payment_id;
                                $feedetails = $this->payment->stampduty_fee_details($token, $lang, $article_id, $request_data['payment_mode_id']);
                                $payment = $this->payment->query("select pay.*,mode.payment_mode_desc_$lang,mode.verification_flag,mode.receipt_flag  FROM ngdrstab_trn_payment_details pay,ngdrstab_mst_payment_mode mode WHERE pay.payment_mode_id=mode.payment_mode_id AND  pay.token_no=?  ", array($token));
                                if (!empty($feedetails)) {
                                    //pr($feedetails); pr($payment); pr($request_data);exit;
...
//exit;
    ...
    function get_payment_details() {
        try {
            $this->check_csrf_token_withoutset($_POST['csrftoken']);
            array_map(array($this, 'loadModel'), array('CitizenPaymentEntry', 'PaymentFields', 'bank_master', 'BankBranch', 'office', 'officehierarchy', 'article_fee_items', 'BankPayment'));
            ...
            foreach ($paymentfields as $field) {
                ...
                if ($field['PaymentFields']['field_name'] == 'bank_trn_id') {
                    $bank_trn_id = $this->BankPayment->find('list', array('fields' => array('transaction_id', 'transaction_id'), 'conditions' => array('payment_mode_id' => $data['mode'], 'token_no' => $token, 'payment_status' => 'SUCCESS')));
    ...
    function final_submit($csrftoken = NULL) {
        try {
            ...
            $lang = $this->Session->read("sess_langauge");
            $doc_lang = $this->Session->read('doc_lang');
            array_map(array($this, 'loadModel'), array('regconfig', 'CitizenUser', 'stamp_duty', 'uploaded_file_trn', 'upload_document', 'smsevent', 'property_details_entry', 'party_category_fields', 'CitizenPaymentEntry', 'article_fee_items', 'article_fee_rule', 'article_fee_subrule', 'party_entry', 'office', 'office_village_map', 'ApplicationSubmitted', 'genernalinfoentry', 'BankPayment', 'conf_reg_bool_info', 'fee_exemption', 'appointment'));
...
//pr($tab); exit;
            ...
            $token_no = $this->Session->read("Selectedtoken");
            $val_amt = $this->party_entry->query("select sum(rounded_val_amt) ,
                                                    p.token_no from ngdrstab_trn_valuation  as val
                                                    JOIN ngdrstab_trn_property_details_entry as p ON p.val_id=val.val_id
                                                    where p.token_no=? group by p.token_no", array($token_no));
            ...
            if ($this->request->is('post')) {
                ...
                if (count($check) <= 0) {
                    ...
                    // supervisor role modifications // change by devashree  
                    /* $this->loadModel("User");
                      $this->loadModel("genernalinfoentry");
                      $getuserid=$this->User->query("select * from ngdrstab_mst_user where office_id='$office_id1' and role_id='100001'");
                      //pr($getuserid);
                      $getuser=$getuserid[0][0]['user_id'];
                      $tokenselect=$this->Session->read("Selectedtoken");
                      $supervisory_flag = $this->regconfig->field('conf_bool_value', array('reginfo_id' => 170));
                      $this->set('supervisory_flag', $supervisory_flag);
                     */
                    //pr($getuser);pr($tokenselect);
                    //exit;
                    ///////////////////////////////////////////////////
                    if ($this->ApplicationSubmitted->save($data)) {

                        // supervisor role modifications // change by devashree  
                        /* if($supervisory_flag=='Y')
                          {
                          $updategeneralinfo=$this->genernalinfoentry->query("update ngdrstab_trn_generalinformation set supervisor_user_id='$getuser' where token_no='$tokenselect'");
                          } */
                        ///////////////////////////////////////////////////
//$appointmentdetails= $this->appointment->find('first', array('conditions' => array('token_no' => $this->Session->read("Selectedtoken"))));
//pr($appointmentdetails);
                        ...
                        $message1 = 'Your document submitted successfully to' . $officename . ' with PreReg No. ' . $this->Session->read("Selectedtoken") . '. Appointment details {#var#}.-NGDRS Portal';
//pr($message1);                    
                        if (!empty($event)) {
                            if ($event[0]['smsevent']['send_flag'] == 'Y') {
...
//                        }
                        ...
                        $home_visit_reg = $this->conf_reg_bool_info->find('first', array('conditions' => array('reginfo_id' => 177)));
                        $home_visit_reg_val = $home_visit_reg['conf_reg_bool_info']['conf_bool_value'];
                        if ($home_visit_reg_val == 'Y') {
                            $party_home_visit = ClassRegistry::init('party_entry')->field('home_visit_flag', array('token_no' => $this->Session->read("Selectedtoken"), 'home_visit_flag' => 'Y'));
                            if (!empty($party_home_visit)) {
                                $this->redirect(array('controller' => 'Citizenentry', 'action' => 'genernal_info'));
                            } else {
...
//            pr($conditions);
    ...
    public function police_station_change_event() {
        try {
            $this->check_csrf_token_withoutset($this->request->data['csrftoken']);
            ...
            if (isset($this->request->data['dist'])) {
                ...
                $doc_lang = $this->Session->read('doc_lang');
                $dist = $this->request->data['dist'];
                $policestation = ClassRegistry::init('PoliceStation')->find('list', array('fields' => array('PoliceStation.policestation_id', 'PoliceStation.stationname_en'), 'conditions' => array('district_id' => $dist)));

                $result_array = array('policestation' => $policestation);
    ...
    function set_token_session($token, $csrftoken = NULL) {
        try {
            ...
            $check = ClassRegistry::init('genernalinfoentry')->find("first", array('conditions' => array('token_no' => $token, 'user_id' => $this->Auth->user('user_id'))));
            if (!empty($check)) {
                ...
                $this->redirect(array('action' => 'citizenlogin', $this->Session->read('csrftoken')));
            }
        } catch (Exception $ex) {
            $this->Session->setFlash(
                    __('Record Cannot be displayed. Error :' . $ex->getMessage())
...
//    }
    ...
    function get_party_feilds() {
        try {
            ...
            $this->check_csrf_token_withoutset($_POST['csrftoken']);
            array_map(array($this, 'loadModel'), array('property_details_entry','conf_reg_bool_info', 'PoliceStation', 'party_entry', 'State', 'maincast', 'marital_status', 'partytype', 'DataEntryReject', 'nationality', 'cast_category', 'gov_partytype', 'District', 'taluka', 'identificatontype', 'party_category_fields', 'bank_master', 'salutation', 'gender', 'occupation', 'presentation_exmp', 'party_category', 'PincodeCSV'));
            $laug = $this->Session->read("sess_langauge");
            $lang = $this->Session->read("sess_langauge");
            $user_id = $this->Session->read("citizen_user_id");
            $token = $this->Session->read('Selectedtoken');
            ...
            $bank_master = $this->bank_master->find('list', array('fields' => array('bank_id', 'bank_name_' . $lang)));
            $executer = array('Y' => 'Yes', 'N' => 'NO');
            $isgovtorg = array('N' => 'NO', 'Y' => 'Yes');
            $home_visit = array('N' => 'NO', 'Y' => 'Yes');
            //$this->set('homevisit', 'N');
            ...
            $policestation = $this->PoliceStation->find('list', array('fields' => array('PoliceStation.policestation_id', 'PoliceStation.stationname_en')));
            $taluka = $this->taluka->find('list', array('fields' => array('id', 'taluka_name_' . $doc_lang), 'order' => array('taluka_name_' . $doc_lang => 'ASC')));
            ...
            $doc_lang = $this->Session->read('doc_lang');
            $fields = $this->set_common_fields();
            $property_list = $this->property_details_entry->get_property_list($doc_lang, $this->Session->read("Selectedtoken"), $fields['user_id']);
            $this->set("property_list", $property_list);
            //pr($property_list);
//validations
            //category flag check
...
//                $partyfields = $this->party_category_fields->find('all', array('fields'=>array(''),'conditions' => array('category_id' => $data['category'], 'is_auth_signtry_field' => 'N', 'display_flag' => 'Y'), 'order' => 'order ASC'));
            ...
            if (isset($data['id']) && is_numeric($data['id'])) {
                ...
                $pinvillages = $this->PincodeCSV->find("list", array(
                    'fields' => array('column_2', 'column_2'),
                    'conditions' => array('column_3' => $party[0]['party_entry']['pin_code'])
                ));
            ...
            $this->set(compact('districtdata', 'policestation', 'allparty', 'home_visit', 'industrial', 'guardian_name', 'permission_applicable', 'stamp_purchaser', 'State', 'maincast', 'category', 'nationality', 'marital_status', 'cast_cat_flag', 'gov_partytype', 'taluka', 'villagelist', 'partyfields', 'identificatontype', 'panlist', 'bank_master', 'executer', 'isgovtorg', 'salutation', 'gender', 'occupation', 'exemption', 'allrule', 'name_format', 'errarr', 'districtdata', 'taluka', 'auth_sign', 'pinvillages'));
        } catch (Exception $ex) {
            pr($ex);
            exit;
            $this->Session->setFlash(
    ...
    public function party_entry($csrftoken = NULL, $tokenval = NULL) {
        try {
            ...
            array_map([$this, 'loadModel'], ['attribute_parameter', 'parameter', 'identificatontype', 'stamp_duty', 'bank_master', 'party_entry', 'party_category', 'property_details_entry', 'salutation', 'gender', 'occupation', 'party_category', 'partytype', 'article_screen_mapping',
            ...
            $nocpara = $this->parameter->query("select * from ngdrstab_trn_parameter where paramter_id='3' and token_id='$tokenval'");
            $this->set('nocpara', $nocpara); //pr($nocpara);
            //
            /// LR integration
            $lr_plot = $this->parameter->query("select property_id,paramter_id,paramter_value from ngdrstab_trn_parameter where paramter_id='1' and token_id='$tokenval'");
            //pr($lr_plot);
            $this->set('lr_plot', $lr_plot);
            $lr_khatian = $this->parameter->query("select property_id,paramter_id,paramter_value from ngdrstab_trn_parameter where paramter_id='2' and token_id='$tokenval'");
            //pr($lr_khatian);
            $this->set('lr_khatian', $lr_khatian);
            /////////////
            
            /// ******* NOC party details *************** ///
            $this->loadModel('trn_noc_land_details');
            $this->loadModel('trn_noc_party_details');
            
            $nocpartydetails = $this->trn_noc_party_details->query("select * from ngdrstab_trn_noc_partydetails where token_no='$tokenval'");
            $this->set('nocpartydetails', $nocpartydetails);
            //pr($nocpartydetails);
            //////////////////////////////////////////////
            //
            //-----------------checking Property Flag and property List---------------------------------            
...
//            $file->write(json_encode($errarr));
            ...
            if ($this->request->is('post')) {
//                pr($this->request->data['party_entry']);exit;
...
//                            spr($this->request->data);exit;
                            ...
                            if (isset($this->request->data['prop_id']) && !empty($this->request->data['prop_id'])) {
                                $prop_ids = $this->request->data['prop_id'];
                            } else {
                                $prop_ids = null;
                            }

                            if ($this->request->data['party_entry']['is_executer'] == 'Y') {
                                $this->request->data['party_entry']['presenty_require'] = 'Y';
                            }
                            if ($this->request->data['party_entry']['is_executer'] == 'N') {
                                $this->request->data['party_entry']['presenty_require'] = 'N';
                            }
//echo $prop_ids;exit;
//                                pr($this->request->data['party_entry']['is_executer']);
//                                exit;

                            if ($this->save_party($this->request->data['party_entry'], $tokenval, $fields['stateid'], $fields['user_id'], $property_id, $hfid, $repeat_party_id = NULL, $prop_ids)) {
                                if ($this->request->data['hfupdateflag'] == 'Y') {
                                    $party_id = $hfid;
                                    $actionvalue = "Updated";
                                } else {
    ...
    function save_party($data, $tokenval, $stateid, $user_id, $property_id, $hfid, $repeat_party_id) {
        array_map([$this, 'loadModel'], ['party_entry']);
        $this->loadModel('party_property_mapping');
        $pan = isset($data['pan_no']) ? ($data['pan_no']) : ('');
        $mobile = isset($data['mobile_no']) ? ($data['mobile_no']) : ('');
        ...
        if (isset($data['poa_id'])) {
            ...
            foreach ($data['poa_id'] as $poa) {
                ...
                $this->party_property_mapping->deleteAll(array('token_no' => $tokenval, 'party_id' => $hfid));

                if (!empty($prop_ids)) {
                    foreach ($prop_ids as $id) {

                        $mapdata = array('party_id' => $hfid,
                            'property_id' => $id,
                            'token_no' => $tokenval,
                            'state_id' => $stateid,
                            'user_id' => $user_id,
                        ...
                        $this->party_property_mapping->saveAll($mapdata);
                    }
                }
            }
            return true;
            ...
            if ($hfid) {
                ...
                $last_party_id = $this->party_entry->getLastInsertID();
                ...
                if (!empty($prop_ids)) {
                    foreach ($prop_ids as $id) {
                        ...
                        $mapdata = array('party_id' => $last_party_id,
                            'property_id' => $id,
                            'token_no' => $tokenval,
                            'state_id' => $stateid,
                            'user_id' => $user_id,
...
//                            
            ...
            if (isset($file) and $file != '') {
                ...
                if (!empty($general)) {
                    ...
                    if (file_exists($path)) {
                        ...
                        $this->response->file($path, array('download' => false, 'name' => $file));
                        //return $this->response->download($file); //// commented on 15 june - devashree - to open pdf in new tab instead of downloading
    ...
    function get_identification_feilds() {
        try {
            ...
            $occupation = $this->occupation->find('list', array('fields' => array('occupation.occupation_id', 'occupation.occupation_name_' . $doc_lang), 'order' => 'occupation_name_' . $doc_lang));
            $exemption = $this->presentation_exmp->find('list', array('fields' => array('presentation_exmp.exemption_id', 'presentation_exmp.desc_' . $doc_lang)));
            $allrule = $this->identificatontype->query('select e.error_code ,e.pattern_rule_client ,e.error_messages_' . $laug . ' from ngdrstab_mst_errorcodes e, ngdrstab_mst_identificationtype i where e.error_code_id=i.error_code_id ');
            $districtdata = $this->District->find('list', array('fields' => array('District.id', 'District.district_name_' . $doc_lang), 'conditions' => array('state_id' => $fields['stateid']), 'order' => 'district_name_' . $doc_lang));
            $taluka = $this->taluka->find('list', array('fields' => array('id', 'taluka_name_' . $doc_lang), 'order' => array('taluka_name_' . $doc_lang => 'ASC')));
...
//                $partyfields = $this->party_category_fields->find('all', array('fields'=>array(''),'conditions' => array('category_id' => $data['category'], 'is_auth_signtry_field' => 'N', 'display_flag' => 'Y'), 'order' => 'order ASC'));
    ...
    public function get_village_bypincode() {
        $this->autoRender = false;
        $this->loadModel('PincodeCSV');

        $pincode = $this->PincodeCSV->find("first", array(
            'fields' => array('column_6', 'column_5', 'column_4'),
            'conditions' => array('column_3' => $this->request->data['pin'])
        ));

        $villages = $this->PincodeCSV->find("list", array(
            'fields' => array('column_2', 'column_2'),
            'conditions' => array('column_3' => $this->request->data['pin'])
        ));
        if (empty($pincode)) {
            $pincode['PincodeCSV']['column_6'] = '';
            $pincode['PincodeCSV']['column_5'] = '';
            $pincode['PincodeCSV']['column_4'] = '';
        }
        $result['master'] = $pincode;
        $result['village'] = $villages;
        echo json_encode($result);
        exit;
    }
    ...
    function get_exmption_rule() {
        try {
            ...
            if (isset($_POST['fee_rule_id']) && is_numeric($_POST['fee_rule_id'])) {
                ...
                $article_id = $this->Session->read('article_id');
                $doc_lang = $this->Session->read('doc_lang');
                $exemption_rule = $this->article_fee_rule->find('list', array('fields' => array('fee_rule_id', 'fee_rule_desc_' . $doc_lang), 'conditions' => array('fee_rule_id' => $this->exemption_article_rules->find('list', array('fields' => array('fee_rule_id'), 'conditions' => array('main_fee_rule_id' => $_POST['fee_rule_id'])))))); //get only Exemption Rules(9998)
//pr($exemption_rule);exit;
                if (empty($exemption_rule)) {
                    $exemption_rule = $this->article_fee_rule->find('list', array('fields' => array('fee_rule_id', 'fee_rule_desc_' . $doc_lang), 'conditions' => array('fee_rule_id' => $this->exemption_article_rules->find('list', array('fields' => array('fee_rule_id'), 'conditions' => array('article_id' => $article_id))), 'main_fee_rule_id' => $_POST['fee_rule_id']))); //get only Exemption Rules(9998) 
                }
...
//            echo '<pre>'; print_r($appt);exit;
    ...
    function reschedule($token, $csrftoken = NULL) {
        try {
            ...
            $home_visit_flag = $this->conf_reg_bool_info->field('conf_bool_value', array('reginfo_id' => 177));
            if ($home_visit_flag == 'Y') {
                $party_home_visit = ClassRegistry::init('party_entry')->field('home_visit_flag', array('token_no' => $token, 'home_visit_flag' => 'Y'));
                if (!empty($party_home_visit)) {
                    $this->Session->setFlash(__('The appointment for Home Visit document is decided by SRO only!!'));
                    $this->redirect(array('action' => 'genernal_info', $this->Session->read('csrftoken')));
                } else {
                    $this->redirect(array('action' => 'appointment', $this->Session->read('csrftoken')));
                }
            } else {
    ...
    function generate_form60($id = null) {
        ...
        $party = $this->party_entry->query('select party_full_name_en,father_full_name_en,uid,mobile_no,pan_no,dob,p.identificationtype_id,p.identificationtype_desc_en as value,iden.identificationtype_desc_en,dist.district_name_en,p.pincode_district_name_en,
p.pincode_taluka_name_en,p.pincode_village_name_en,p.address_en,tal.taluka_name_en,village.village_name_en
...
where p.id=' . $id
        ...
        $html_design = "<h1 align=center> Form 60/61 <br/></h1> "
                ...
                . "<tr> <td width=50%> <b><h3> District Name : </b> </td> <td align=center> " . ($party[0][0]['district_name_en']) . "" . ($party[0][0]['pincode_district_name_en']) . "</td> </tr>"
                . "<tr> <td width=50%> <b><h3> Taluka Name : </b> </td> <td align=center>  " . ($party[0][0]['taluka_name_en']) . "" . ($party[0][0]['pincode_taluka_name_en']) . "</td> </tr>"
                . "<tr> <td width=50%> <b><h3> Village Name : </b> </td> <td align=center>  " . ($party[0][0]['village_name_en']) . "" . ($party[0][0]['pincode_village_name_en']) . "</td> </tr>"
                . "<tr> <td width=50%> <b> <h3>Address : </b> </td> <td align=center>  " . implode(', ', $party_address) . " " . $party[0][0]['address_en'] . "</td> </tr>"
                . "</table><br/>";

        $html_design .= "<br><br><h3 align=right>Signature</h3>";

        $this->create_pdf($html_design, 'Form_60_61', 'A4', 'Form 60/61');
    }
...
//            );
    ...
    function getlrdataonpartytripura() {
        try {
            //$this->autoRender=FALSE;
            $this->loadModel('User');
            $this->loadModel('VillageMapping');
            ...
            /* $villageid = $data['village'];
              $district = $data['district'];
              $sqlf = $this->VillageMapping->query("select * from ngdrstab_conf_admblock7_village_mapping where district_id='$district' and village_id='$villageid'");
              $igd_village_code = $sqlf[0][0]['census_code'];

             */
            $tokeninst = $this->Session->read('Selectedtoken');
            $fetchdetails = $this->property_details_entry->query("select * from ngdrstab_trn_property_details_entry where token_no='$tokeninst' and property_id='$propid'");
            //pr($fetchdetails);
            $district = $fetchdetails[0][0]['district_id'];
            $villageid = $fetchdetails[0][0]['village_id'];
            $sqlf = $this->VillageMapping->query("select * from ngdrstab_conf_admblock7_village_mapping where district_id='$district' and village_id='$villageid'");
            $census_code_frm = $sqlf[0][0]['census_code'];

            //for khatian entry
            /* $para_details=$this->parameter->query("select * from ngdrstab_trn_parameter where token_id='$tokeninst' and property_id='$propid' and paramter_id=2");
              $paraval=$para_details[0][0]['paramter_value'];
             */

            //for plot entry
            $para_details = $this->parameter->query("select * from ngdrstab_trn_parameter where token_id='$tokeninst' and property_id='$propid' and paramter_id=1");
            if (sizeof($para_details) > 0) {
                $paraval = $para_details[0][0]['paramter_value'];
            } else {
                echo '0';
                exit;
            ...
            /* $flgkhatian='N';
              pr($json2array);exit;
              if (isset($json2array['prop_attributes_seller'])) {
              for($s=0;$s<sizeof($json2array['prop_attributes_seller']);$s++){
              $attrid=$json2array['prop_attributes_seller'][$s]['attribute_id'];
              if($attrid==2)
              {
              $khatian_number=$json2array['prop_attributes_seller'][$s]['attribute_value'];
              $flgkhatian='Y';
              }
            ...
            $igd_village_code = $census_code_frm;
            //for khatian
            //$khatian_no = $paraval;
            //for plot
            $plot_no = $paraval;
            $this->loadModel('external_interface');
            //for khatian
            //$interface = $this->external_interface->find('all', array('conditions' => array('external_interface.interface_id ' => 37)));
            //for plot
            $interface = $this->external_interface->find('all', array('conditions' => array('external_interface.interface_id ' => 36)));

            $url = $interface[0]['external_interface']['interface_url'];
            $app_key = $interface[0]['external_interface']['secure_key'];
            $application_id = $interface[0]['external_interface']['interface_user_id'];

            //for khatian
            //$hmacstring = $igd_village_code . $khatian_no . $application_id;
            //for plot
            $hmacstring = $igd_village_code . $plot_no . $application_id;

            $hmac_encrypted = hash_hmac('sha256', $hmacstring, $app_key, true);
            $hmac = base64_encode($hmac_encrypted);
            //$serviceurl= $url.$igd_village_code . '/' . $khatian_no . '/' . $application_id . '/' . $hmac;
            //for khatian
            //$serviceurl= $url.'lgd_village_code='.$igd_village_code . '&khatian_no=' . $khatian_no . '&application_id=' . $application_id . '&hmac=' . $hmac;
            //for plot
            $serviceurl = $url . 'lgd_village_code=' . $igd_village_code . '&plot_no=' . $plot_no . '&application_id=' . $application_id . '&hmac=' . $hmac;
//pr($serviceurl);
            $contents = file_get_contents($serviceurl);
            $lrdata = json_decode($contents, true);
            //pr($lrdata);
            $this->set('lrdata', $lrdata);
        } catch (Exception $ex) {
            pr($ex);
            exit;
        }
    ...
    public function getnocvalue() {
        ...
        if (isset($json2array['prop_attributes_seller'])) {
            ...
            for ($s = 0; $s < sizeof($json2array['prop_attributes_seller']); $s++) {
                $attrid = $json2array['prop_attributes_seller'][$s]['attribute_id'];
                //pr($attrid);
                //pr($json2array['prop_attributes_seller'][$s]['attribute_value']);
                if ($attrid == '3') {
                    $nocval = $json2array['prop_attributes_seller'][$s]['attribute_value'];
                } else {
                    $nocval = 0;
                }
            }
    ...
    public function getnocview() {

        if (isset($_POST['noc_number'])) {
            $nocval = $_POST['noc_number'];
            //pr($nocval);
            //exit;
        } else {
            $nocval = 0;
        ...
        $this->set('nocval', $nocval);
        //echo $nocval;
        //NOC/07/30146/2021
    }

    public function viewnoc_in_ngdrs() {
        //pr($_POST);
        $pdf_decoded = $_POST['data'];
//        pr(base64_decode($pdf_decoded));exit;
        header('Content-Type: application/pdf');
        pr(base64_decode($pdf_decoded));
        exit;
    }

    public function getnocpartydetails() {
        /* $nocval=$_POST['nocval'];
          //pr($nocval);
          $data_string=array(
          'data' =>  $nocval
          ...
          pr($data_string);
          $url="https://ilrms.nic.in/noc/index.php/nocwebservice/seller";
          $ch = curl_init($url);
          curl_setopt($ch, CURLOPT_POSTFIELDS, $data_string);
          curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
          ...
          exit; */
        $nocval = 'NOC/07/30146/2021';
        $this->set('nocval', $nocval);
    }

    public function viewnoc_partydetails_in_ngdrs() {
        //$token = $this->Session->read('Selectedtoken');
        //pr($token);
        ...
        if (isset($_POST)) {
            $this->loadModel('trn_noc_land_details');
            $this->loadModel('trn_noc_party_details');
            //pr($_POST);//echo 'aaaaaaaaaaaaaaaaaaan';
            $data = $_POST['data'];
            $tokendd = $this->Session->read("Selectedtoken");
            //pr($data);
            //pr(base64_decode($data));
            $decoded_data = base64_decode($data);
            $json_decoded = json_decode($decoded_data);
            //pr($tokendd);
            //pr($json_decoded);exit;
            $this->set('json_decoded', $json_decoded);
            //pr($json_decoded);
            if (is_object($json_decoded)) {
                $bb = get_object_vars($json_decoded);
                //pr($bb);
                //pr(sizeof($bb['nocdata']));
                for ($i = 0; $i < sizeof($bb['nocdata']); $i++) {
                    if (is_object($bb['nocdata'][$i])) {
                        $cc = get_object_vars($bb['nocdata'][$i]);
                        //pr($cc);

                        $html_body = "<table style='width:90%;border-collapse: collapse;'><tr style='border: 1px solid;background-color:#6485a2;color:#fff;padding-right: 15px;padding-left: 15px;'><th style='border: 1px solid;'>App No</th><th style='border: 1px solid;'>combkey</th></tr>";
                        $html_body .= "<tr style='border: 1px solid;'><td style='border: 1px solid;'>" . $cc['appno'] . "</td><td style='border: 1px solid;'>" . $cc['combkey'] . "</td></tr>";
                        $html_body .= "</table><br>";
                        pr($html_body);
                    }
                }
                for ($j = 0; $j < sizeof($bb['seller']); $j++) {
                    if (is_object($bb['seller'][$j])) {
                        $dd = get_object_vars($bb['seller'][$j]);

                        $html_body_two = "<table style='width:100%;border-collapse: collapse;'><caption style='text-align:centre;color:red;font-weight:bold;'>Seller Data</caption><tr style='border: 1px solid;background-color:#6485a2;color:#fff;'><th style='border: 1px solid;'>slno</th><th style='border: 1px solid;'>sname</th><th style='border: 1px solid;'>sfname</th><th style='border: 1px solid;'>prhouse</th><th style='border: 1px solid;'>prlocality</th><th style='border: 1px solid;'>prpin</th><th style='border: 1px solid;'>prpoff</th><th style='border: 1px solid;'>prdist</th></tr>";
                        $html_body_two .= "<tr style='border: 1px solid;'><td style='border: 1px solid;'>" . $dd['slno'] . "</td><td style='border: 1px solid;'>" . $dd['sname'] . "</td><td style='border: 1px solid;'>" . $dd['sfname'] . "</td><td>" . $dd['prhouse'] . "</td><td style='border: 1px solid;'>" . $dd['prlocality'] . "</td><td style='border: 1px solid;'>" . $dd['prpin'] . "</td><td style='border: 1px solid;'>" . $dd['prpoff'] . "</td><td style='border: 1px solid;'>" . $dd['prdist'] . "</td></tr>";
                        $html_body_two .= "</table><br>";

                        $slctrecord_fromland = $this->trn_noc_party_details->query("select * from ngdrstab_trn_noc_partydetails where token_no=? and appno=?", array($this->Session->read("Selectedtoken"), $cc['appno']));
                        //pr($slctrecord_fromland);exit;
                        if (sizeof($slctrecord_fromland) <= 0) {
                            $this->trn_noc_party_details->query("insert into ngdrstab_trn_noc_partydetails(token_no, appno, combkey, party_type, name, fname, prhouse, prlocality, prpin, prpoff, prdist,party_instdate) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", array($this->Session->read("Selectedtoken"), $cc['appno'], $cc['combkey'], 'seller', $dd['sname'], $dd['sfname'], $dd['prhouse'], $dd['prlocality'], $dd['prpin'], $dd['prpoff'], $dd['prdist'], 'now()'));
                        } else {
                            $this->trn_noc_party_details->query("update ngdrstab_trn_noc_partydetails set combkey=?, party_type=?, name=?, fname=?, prhouse=?, prlocality=?, prpin=?, prpoff=?, prdist=?, party_instdate=? where token_no=? and appno=?", array($cc['combkey'], 'seller', $dd['sname'], $dd['sfname'], $dd['prhouse'], $dd['prlocality'], $dd['prpin'], $dd['prpoff'], $dd['prdist'], 'now()', $this->Session->read("Selectedtoken"), $cc['appno']));
                        }

                        pr($html_body_two);
                    }
                }
                for ($k = 0; $k < sizeof($bb['buyer']); $k++) {
                    if (is_object($bb['buyer'][$k])) {
                        $ee = get_object_vars($bb['buyer'][$k]);

                        $html_body_three = "<table style='width:100%;border-collapse: collapse;'><caption style='text-align:centre;color:red;font-weight:bold;'>Buyer Data</caption><tr style='border: 1px solid;background-color:#6485a2;color:#fff;'><th style='border: 1px solid;'>slno</th><th style='border: 1px solid;'>sname</th><th style='border: 1px solid;'>sfname</th><th style='border: 1px solid;'>prhouse</th><th style='border: 1px solid;'>prlocality</th><th style='border: 1px solid;'>prpin</th><th style='border: 1px solid;'>prpoff</th><th style='border: 1px solid;'>prdist</th></tr>";
                        $html_body_three .= "<tr style='border: 1px solid;'><td style='border: 1px solid;'>" . $ee['slno'] . "</td><td style='border: 1px solid;'>" . $ee['bname'] . "</td><td style='border: 1px solid;'>" . $ee['bfname'] . "</td><td style='border: 1px solid;'>" . $ee['prhouse'] . "</td><td style='border: 1px solid;'>" . $ee['prlocality'] . "</td><td style='border: 1px solid;'>" . $ee['prpin'] . "</td><td style='border: 1px solid;'>" . $ee['prpoff'] . "</td><td style='border: 1px solid;'>" . $ee['prdist'] . "</td></tr>";
                        $html_body_three .= "</table><br>";

                        $slctrecord_fromland = $this->trn_noc_party_details->query("select * from ngdrstab_trn_noc_partydetails where token_no=? and appno=?", array($this->Session->read("Selectedtoken"), $cc['appno']));
                        if (sizeof($slctrecord_fromland) <= 0) {
                            $this->trn_noc_party_details->query("insert into ngdrstab_trn_noc_partydetails(token_no, appno, combkey, party_type, name, fname, prhouse, prlocality, prpin, prpoff, prdist,party_instdate) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", array($this->Session->read("Selectedtoken"), $cc['appno'], $cc['combkey'], 'buyer', $ee['bname'], $ee['bfname'], $ee['prhouse'], $ee['prlocality'], $ee['prpin'], $ee['prpoff'], $ee['prdist'], 'now()'));
                        } else {
                            $this->trn_noc_party_details->query("update ngdrstab_trn_noc_partydetails set combkey=?, party_type=?, name=?, fname=?, prhouse=?, prlocality=?, prpin=?, prpoff=?, prdist=?, party_instdate=? where token_no=? and appno=?", array($cc['combkey'], 'buyer', $ee['bname'], $ee['bfname'], $ee['prhouse'], $ee['prlocality'], $ee['prpin'], $ee['prpoff'], $ee['prdist'], 'now()', $this->Session->read("Selectedtoken"), $cc['appno']));
                        }

                        pr($html_body_three);
                    }
                }
                for ($l = 0; $l < sizeof($bb['landschedule']); $l++) {
                    if (is_object($bb['landschedule'][$l])) {
                        $ff = get_object_vars($bb['landschedule'][$l]);

                        $html_body_four = "<table style='width:100%;border-collapse: collapse;'><tr style='border: 1px solid;background-color:#6485a2;color:#fff;'><th style='border: 1px solid;'>appslno</th><th style='border: 1px solid;'>lncombkey</th><th style='border: 1px solid;'>bigha</th><th style='border: 1px solid;'>katha</th><th style='border: 1px solid;'>lessa</th><th style='border: 1px solid;'>are</th><th style='border: 1px solid;'>acre</th><th style='border: 1px solid;'>sqfeet</th><th style='border: 1px solid;'>pattano</th><th style='border: 1px solid;'>dagno</th><th style='border: 1px solid;'>Ganda</th><th style='border: 1px solid;'>Patta Type</th></tr>";
                        $html_body_four .= "<tr style='border: 1px solid;'><td style='border: 1px solid;'>" . $ff['appslno'] . "</td><td style='border: 1px solid;'>" . $ff['lncombkey'] . "</td><td style='border: 1px solid;'>" . $ff['bigha'] . "</td><td style='border: 1px solid;'>" . $ff['katha'] . "</td><td style='border: 1px solid;'>" . $ff['lessa'] . "</td><td style='border: 1px solid;'>" . $ff['are'] . "</td><td style='border: 1px solid;'>" . $ff['acre'] . "</td><td style='border: 1px solid;'>" . $ff['sqfeet'] . "</td><td style='border: 1px solid;'>" . $ff['pattano'] . "</td><td style='border: 1px solid;'>" . $ff['dagno'] . "</td><td style='border: 1px solid;'>" . $ff['ganda'] . "</td><td style='border: 1px solid;'>" . $ff['pattatype'] . "</td></tr>";
                        $html_body_four .= "</table><br>";

                        $slctrecord_fromland = $this->trn_noc_land_details->query("select * from ngdrstab_trn_noc_landdetails where token_no=? and appno=?", array($this->Session->read("Selectedtoken"), $cc['appno']));
                        if (sizeof($slctrecord_fromland) <= 0) {
                            $this->trn_noc_land_details->query("insert into ngdrstab_trn_noc_landdetails(token_no,appno,combkey,appslno,lncombkey,bigha,katha,lessa,are,acre,sqfeet,pattano,dagno,ganda,pattatype,land_instdate) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", array($this->Session->read("Selectedtoken"), $cc['appno'], $cc['combkey'], $ff['appslno'], $ff['lncombkey'], $ff['bigha'], $ff['katha'], $ff['lessa'], $ff['are'], $ff['acre'], $ff['sqfeet'], $ff['pattano'], $ff['dagno'], $ff['ganda'], $ff['pattatype'], 'now()'));
                        } else {
                            $this->trn_noc_land_details->query("update ngdrstab_trn_noc_landdetails set combkey=?,appslno=?,lncombkey=?,bigha=?,katha=?,lessa=?,are=?,acre=?,sqfeet=?,pattano=?,dagno=?,ganda=?,pattatype=?,land_instdate=? where token_no=? and appno=?", array($cc['combkey'], $ff['appslno'], $ff['lncombkey'], $ff['bigha'], $ff['katha'], $ff['lessa'], $ff['are'], $ff['acre'], $ff['sqfeet'], $ff['pattano'], $ff['dagno'], $ff['ganda'], $ff['pattatype'], 'now()', $this->Session->read("Selectedtoken"), $cc['appno']));
                        }
                        pr($html_body_four);
                    }
                }
            ...
            $this->set('json_decoded', '');
            echo 'No data posted.';
            exit;
        }
    }
    public function viewnoc_partydetails_landhub(){
        try {
            
            $this->loadModel('trn_noc_land_details');
            $this->loadModel('trn_noc_party_details');
            $this->loadModel('article_partymapping');
            $appno = $_POST['noc_number'];
            $tokenval = $_POST['tokenval'];
            //$appno = 'NOC/07/37843/2022';  /// for multiple seller
            //$appno = 'NOC/07/37100/2022';   // for multiple seller and multiple buyer
            $data_encoded = array(
                'apikey' => 'abcd123haryanasinglesigonapplicationDFFEFSDAFETUVX',
                'appno' => $appno
            );
            
            $article_id = $this->Session->read('article_id');
            
            $buyerpartytype = $this->article_partymapping->query("SELECT * FROM public.ngdrstab_mst_article_partytype_mapping one inner join ngdrstab_mst_party_type two on 
            one.party_type_id = two.party_type_id WHERE one.article_id='$article_id' and two.party_type_flag='0'");
            if(sizeof($buyerpartytype)>0){
                $buyerpartytype = $buyerpartytype[0][0]['party_type_id'];
            }
            else{
                $buyerpartytype = 10;
            }
            
            $sellerpartytype = $this->article_partymapping->query("SELECT * FROM public.ngdrstab_mst_article_partytype_mapping one inner join ngdrstab_mst_party_type two on 
            one.party_type_id = two.party_type_id WHERE one.article_id='$article_id' and two.party_type_flag='1'");
            if(sizeof($sellerpartytype)>0){
                $sellerpartytype = $sellerpartytype[0][0]['party_type_id'];
            }
            else{
                $sellerpartytype = 9;
            }
            
            $this->set('buyerpartytype', $buyerpartytype);
            $this->set('sellerpartytype', $sellerpartytype);
            
            //pr($data_encoded);
            $url = 'https://landhub.assam.gov.in/api/index.php/NicApi/seller';
            $ch = curl_init($url);
            ...
            if ($result1) {
                ...
                //echo '1';
                //pr($result1);exit;
                $json_decoded=json_decode($result1);
                $this->set('json_decoded', $json_decoded);
                if (is_object($json_decoded)) {
                    $bb = get_object_vars($json_decoded);
                    //pr($bb);
                    if (is_object($bb['data'])) {
                    $cc = get_object_vars($bb['data']);
                        //pr($cc);exit;
                        $this->set('cc', $cc);
                        
                        $this->trn_noc_party_details->query("delete from  ngdrstab_trn_noc_partydetails where token_no=? and appno=?", array($tokenval, $appno));
                        $this->trn_noc_land_details->query("delete from ngdrstab_trn_noc_landdetails where token_no=? and appno=?", array($tokenval, $appno));
                        
                        if (is_object($cc['nocdata'])) {
                            $dd = get_object_vars($cc['nocdata']);
                            $this->set('dd', $dd);
                        }
                        
                        
                        $ee=array();
                        for($a=0;$a<sizeof($cc['seller']);$a++){
                            if (is_object($cc['seller'][$a])) {
                                $ee[$a] = get_object_vars($cc['seller'][$a]);
                                
                                //$slctrecord_fromland = $this->trn_noc_party_details->query("select * from ngdrstab_trn_noc_partydetails where token_no=? and appno=?", array($tokenval, $appno));
                                //pr($slctrecord_fromland);exit;
                                //if (sizeof($slctrecord_fromland) <= 0) {
                                    $this->trn_noc_party_details->query("insert into ngdrstab_trn_noc_partydetails(token_no, appno, combkey, party_type, name, fname, prhouse, prlocality, prpin, prpoff, prdist,party_instdate) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", array($tokenval, $dd['appno'], $dd['combkey'], 'seller', $ee[$a]['sname'], $ee[$a]['sfname'], $ee[$a]['prhouse'], $ee[$a]['prlocality'], $ee[$a]['prpin'], $ee[$a]['prpoff'], $ee[$a]['prdist'], 'now()'));
                                //} else {
                                  //  $this->trn_noc_party_details->query("update ngdrstab_trn_noc_partydetails set combkey=?, party_type=?, name=?, fname=?, prhouse=?, prlocality=?, prpin=?, prpoff=?, prdist=?, party_instdate=? where token_no=? and appno=?", array($dd['combkey'], 'seller', $ee[$a]['sname'], $ee[$a]['sfname'], $ee[$a]['prhouse'], $ee[$a]['prlocality'], $ee[$a]['prpin'], $ee[$a]['prpoff'], $ee[$a]['prdist'], 'now()', $tokenval, $dd['appno']));
                                //}
                        ...
                        $ff=array();
                        for($q=0;$q<sizeof($cc['buyer']);$q++){
                            if (is_object($cc['buyer'][$q])) {
                                $ff[$q] = get_object_vars($cc['buyer'][$q]);
                                
                                //$slctrecord_fromland = $this->trn_noc_party_details->query("select * from ngdrstab_trn_noc_partydetails where token_no=? and appno=?", array($tokenval, $appno));
                                //if (sizeof($slctrecord_fromland) <= 0) {
                                    $this->trn_noc_party_details->query("insert into ngdrstab_trn_noc_partydetails(token_no, appno, combkey, party_type, name, fname, prhouse, prlocality, prpin, prpoff, prdist,party_instdate) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", array($tokenval, $dd['appno'], $dd['combkey'], 'buyer', $ff[$q]['bname'], $ff[$q]['bfname'], $ff[$q]['prhouse'], $ff[$q]['prlocality'], $ff[$q]['prpin'], $ff[$q]['prpoff'], $ff[$q]['prdist'], 'now()'));
                                //} else {
                                  //  $this->trn_noc_party_details->query("update ngdrstab_trn_noc_partydetails set combkey=?, party_type=?, name=?, fname=?, prhouse=?, prlocality=?, prpin=?, prpoff=?, prdist=?, party_instdate=? where token_no=? and appno=?", array($dd['combkey'], 'buyer', $ff[$q]['bname'], $ff[$q]['bfname'], $ff[$q]['prhouse'], $ff[$q]['prlocality'], $ff[$q]['prpin'], $ff[$q]['prpoff'], $ff[$q]['prdist'], 'now()', $tokenval, $dd['appno']));
                                //}
                        ...
                        $gg=array();
                        for($e=0;$e<sizeof($cc['landschedule']);$e++){
                            if (is_object($cc['landschedule'][$e])) {
                                $gg[$e] = get_object_vars($cc['landschedule'][$e]); 
                                
                               // $slctrecord_fromland = $this->trn_noc_land_details->query("select * from ngdrstab_trn_noc_landdetails where token_no=? and appno=?", array($tokenval, $appno));
                                //if (sizeof($slctrecord_fromland) <= 0) {
                                    $this->trn_noc_land_details->query("insert into ngdrstab_trn_noc_landdetails(token_no,appno,combkey,appslno,lncombkey,bigha,katha,lessa,are,acre,sqfeet,pattano,dagno,ganda,pattatype,land_instdate,consideration_amount,district,unique_village_code) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", array($tokenval, $dd['appno'], $dd['combkey'], $gg[$e]['appslno'], $gg[$e]['lncombkey'], $gg[$e]['bigha'], $gg[$e]['katha'], $gg[$e]['lessa'], $gg[$e]['are'], $gg[$e]['acre'], $gg[$e]['sqfeet'], $gg[$e]['pattano'], $gg[$e]['dagno'], $gg[$e]['ganda'], $gg[$e]['pattatype'], 'now()',$dd['considerationamount'],$gg[$e]['district'],$gg[$e]['unique_village_code']));
                               // } else {
                                //    $this->trn_noc_land_details->query("update ngdrstab_trn_noc_landdetails set combkey=?,appslno=?,lncombkey=?,bigha=?,katha=?,lessa=?,are=?,acre=?,sqfeet=?,pattano=?,dagno=?,ganda=?,pattatype=?,land_instdate=? where token_no=? and appno=?", array($dd['combkey'], $gg[$e]['appslno'], $gg[$e]['lncombkey'], $gg[$e]['bigha'], $gg[$e]['katha'], $gg[$e]['lessa'], $gg[$e]['are'], $gg[$e]['acre'], $gg[$e]['sqfeet'], $gg[$e]['pattano'], $gg[$e]['dagno'], $gg[$e]['ganda'], $gg[$e]['pattatype'], 'now()', $tokenval, $dd['appno']));
                                //}
                            } 
                        }
                        $this->set('gg', $gg);
    ...
    public function check_duplicate_pan(){
        $this->loadModel('party_entry');
        $panno=$_POST['pan_no'];
        $token = $this->Session->read('Selectedtoken');
        $recexist = $this->party_entry->query("select * from ngdrstab_trn_party_entry_new where token_no='$token' and pan_no='$panno'");
        if(sizeof($recexist)>0){
            echo '1';
        }
        else{
            echo '0';
    ...
    public function check_duplicate_mobile(){
        ...
        $token = $this->Session->read('Selectedtoken');
        $recexist = $this->party_entry->query("select * from ngdrstab_trn_party_entry_new where token_no='$token' and mobile_no='$mobileno'");
        if(sizeof($recexist)>0){
            echo '1';
        }
        else{
